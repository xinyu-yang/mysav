...
class SwitchControl:
    ...
    def syncTime(self, time, modify=True):
        table_entry = self.p4info_helper.buildTableEntry(
            table_name="MyIngress.getTime_lpm",
            match_fields={
                "hdr.ip.v4.version": (0, 1)
            },
            action_name="MyIngress.getTime",
            action_params={
                "time": time
            })
        self.sw.WriteTableEntry(table_entry, modify=modify)
        print "synced time on %s" % self.sw_name

    def writeTableRules(self, modify):
        """
...
def main(topology_file_path, sw):
    ...
    while True:
        ...
        print "\n************syncing time************"
        ctime = int(time()*1000).__mod__(65536)
        if p4prgm[sw] != "basic":
            try:
                swc.syncTime(ctime)
                print "-------synced %s-------" % sw
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--topology', help='Topology JSON file',
                        type=str, action="store", required=False,
                        default='./topology.json')
    parser.add_argument('-s', '--switch', help='switch',
                        type=str, action="store", required=False,
                        default='s1')
    ...
    #     parser.print_help()
    #     print "\n p4info file not found: %s\n Have you run 'make'?" % args.switch
    #     parser.exit(1)
    # if not os.path.exists(args.bmv2_json):
    #     parser.print_help()
    #     print "\nBMv2 JSON file not found: %s\nHave you run 'make'?" % args.bmv2_json
    #     parser.exit(1)
    main(args.topology, args.switch)
